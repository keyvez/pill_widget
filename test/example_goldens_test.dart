...
void main() {
  ...
  Widget buildTestWrapper(Widget child, {required Brightness brightness}) {
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        backgroundColor: isLight ? const Color(0xFFE0E0E0) : const Color(0xFF424242),
        body: Center(
          child: RepaintBoundary(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isLight ? const Color(0xFFF5F5F5) : const Color(0xFF2C2C2C),
  ...
  // Define all screenshot scenarios
  final scenarios = <String, Widget Function()>{
    'basic_pills': () => const Wrap(
      spacing: 8,
      runSpacing: 8,
      children: [
        Pill(label: 'Active'),
    ...
    'styled_values_pills': () => const Wrap(
      spacing: 8,
      runSpacing: 8,
      children: [
        Pill(
  ...
  // Generate light and dark golden tests for each scenario
  for (final entry in scenarios.entries) {
    final name = entry.key;
    final builder = entry.value;

    testWidgets('$name Light Golden', (tester) async {
      await tester.pumpWidget(
        buildTestWrapper(builder(), brightness: Brightness.light),
      );
      await tester.pumpAndSettle();
      await expectLater(
        find.byKey(const ValueKey('golden-boundary')),
        matchesGoldenFile('../screenshots/light/$name.png'),
      );
    });

    testWidgets('$name Dark Golden', (tester) async {
      await tester.pumpWidget(
        buildTestWrapper(builder(), brightness: Brightness.dark),
      );
      await tester.pumpAndSettle();
      await expectLater(
        find.byKey(const ValueKey('golden-boundary')),
        matchesGoldenFile('../screenshots/dark/$name.png'),
      );
    });
  }
}
